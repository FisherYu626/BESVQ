...
def Build_index(Kw_File_Use):
    ...
    for kw in Kw_File_Use:
        ...
        # print(st)
        start_file_addr = hmac.new(st).digest()  # client存储的首文件地址 F(w||f_i)
        ...
        # print("Before encryption: ", len(t_addr))
        # print("After encryption: ", len(Toekn_S_Addr))
        ...
        for i in range(len(Kw_File_Use[kw])):
           if i==0:  #nonce块
               ...
               addr = hmac.new(str1)
               addr_file = addr.digest()    # ptr_0 = F(w||\alpha_w)
               w_f_addr_table[kw]=[addr_file]

               ################生成当前G
               # G(t1w,ptr n)
               aes_t1 = AES.new(Toekn_S_Addr, model)    
               G_token_S_addr = aes_t1.encrypt(addr_file) # P_0 = G(t_w^1,ptr_1)
               # G(t2w,ptr n)
...
#    address=w3.toChecksumAddress('0x8689a5B6F636F75BC66a89F01F3D0DA3eFFB1332'),
#    abi=abi_build_index)
...
endb = time.time() 
print("build index time", endb-startb, " s")
...
#         tx_hash11=store_var_contract.functions.setbatch(batchtoken, batchhash,500).transact({
#             "from": from_account,
#             "gas": 3000000,
#             "gasPrice": 0,
#         })
...
print(len(blockchain_addfile_index))
...
print(list_search_file_ID1)
print(len(list_search_file_ID1))


print("*************file ID***************")
aes = AES.new(secreat_key, model) 
for enc_file_ID in list_search_file_ID1:
	fileID_en=aes.decrypt(enc_file_ID) # C_f
	# m=bytes.decode(fileID_en)
	print(fileID_en)
